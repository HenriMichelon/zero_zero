...
void Menu::onMenuQuit(GWidget &, GEvent *) {
    Application::quit();
}

void Menu::onMenuTriangle(GWidget &, GEvent *) {
    //hide();
    Application::get().setRootNode(make_shared<TriangleMainScene>());
}

void Menu::onMenuAddRemoveChild(GWidget &, GEvent *) {
    //hide();
...
void Menu::onCreate() {
    ...
    entryTriangle->connect(GEvent::OnClick, this, GEventFunction(&Menu::onMenuTriangle));
    getWidget().add(entryTriangle, GWidget::TOPCENTER);

    auto entryAddRemoveChild = make_shared<GMenuEntry>("Add & remove child");
    entryAddRemoveChild->connect(GEvent::OnClick, this, GEventFunction(&Menu::onMenuAddRemoveChild));
    getWidget().add(entryAddRemoveChild, GWidget::TOPCENTER);

    auto entryQuit = make_shared<GMenuEntry>("Quit");
    entryQuit->connect(GEvent::OnClick, this, GEventFunction(&Menu::onMenuQuit));
