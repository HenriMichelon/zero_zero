...
module z0.vulkan.DiffuseFrameBuffer;
...
namespace z0 {
    ...
    DiffuseFrameBuffer::DiffuseFrameBuffer(const Device &dev, const bool isMultisampled):
        SampledFrameBuffer{dev},
        multisampled{isMultisampled} {
        DiffuseFrameBuffer::createImagesResources();
    }

    void DiffuseFrameBuffer::createImagesResources() {
        createImage(
                device.getSwapChainExtent().width,
                device.getSwapChainExtent().height,
                DIFFUSE_BUFFER_FORMATS[static_cast<int>(app().getConfig().diffuseBufferFormat)],
    ...
    void DiffuseFrameBuffer::cleanupImagesResources() {
