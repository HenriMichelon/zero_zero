    ...
    void ShadowMapRenderer::updateLightSpace() {
        if (lightIsDirectional) {
            ...
            for (uint32_t i = 0; i < ShadowMapFrameBuffer::CASCADED_SHADOWMAP_LAYERS; i++) {
                ...
                // const auto lightDir = normalize(mat3{directionalLight->getTransformGlobal()} * directionalLight->getDirection());
                ...
                const auto limit    = splitDepth[i] - lastSplitDist;
                cameraPosition.z -= lastSplitDist;
                ...
                auto sceneCenter      = (sceneMin + sceneMax) / 2.0f;
                auto lightPosition    = sceneCenter - lightDirection * (orthoDepth/2.0f);
                // Position is scene center offset by light direction
                auto lightProjection = ortho(-orthoWidth / 2, orthoWidth / 2,
                                        -orthoHeight / 2, orthoHeight / 2,
                                        -orthoDepth, orthoDepth);
                lightSpace[i] = lightProjection * lookAt(lightPosition, sceneCenter, AXIS_UP);
